...
router.get('/ack', async (req, res, next) => {
  ...
  <i class="bx bx-heart like-icon" style="color: ${a.like.includes(output.data.login)?'red':'white'}" onclick="ulike('${a.cmd}','${output.data.login}')"></i>
  <div class="like-count">${a.like.length}</div>
  <div class="text-maker">${a.cmd}<span class="by"> by </span><span class="dev" onclick="window.location.href='${a.creator.u}';">${a.creator.n}</span></div>
