...
router.get('/ack', async (req, res, next) => {
    ...
    res.sendFile(__path + '/plugin/login.html')
        ...
        const json = outp.data.map(a=> `<div><a href="${a.url}">${a.cmd}</a><br><p>${a.desc}</p><br><a href="${a.creator.u}">${a.creator.n}</a><br><br><button onclick=u(${a.cmd},${output.data.login})>likes: ${a.like.length}</button>`);
