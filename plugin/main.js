...
router.get('/ack', async (req, res, next) => {
    ...
    res.sendFile(__path + '/plugin/login.html')
        ...
        await fs.writeFileSync(__path + `/public/${id}.html`,`<html><head></head><title>Age Calculator</title><link rel="stylesheet" href="/style.css"><body><p>${output.data.login}</p><br><img src="${output.data.avatar_url}"><br><br>${json}<br><p id="like">clicked</p><script src="https://cdnjs.cloudflare.com/ajax/libs/axios/1.0.0-alpha.1/axios.min.js"></script><script src="/func.js"></script></body></html>`);
