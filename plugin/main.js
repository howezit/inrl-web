...
router.get('/ack', async (req, res, next) => {
    ...
    res.sendFile(__path + '/plugin/login.html')
        ...
        const change = () => { document.getElementById("s").value= "emdi" };
        await fs.writeFileSync(__path + `/plugin/store/${id}.html`,`<html><body><p>${output.data.login}</p><br><img src="${output.data.avatar_url}"><br><br>${json}<br><p id="s">clicked</p><script>${change}</script></body></html>`);
