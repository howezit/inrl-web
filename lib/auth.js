...
const {sendMail,sendWa} = require('./toclient');
...
const saveConnections = db.define('login', {
	key: {
...
async function saveLogin(key) {
	const exist = await saveConnections.findOne({
		where: {
			key
		}
	});
	if(!exist) return await saveConnections.create({key});
	return true;
	}
async function getLogin(key) {
	const exist = await saveConnections.findOne({
		where: {
			key
		}
