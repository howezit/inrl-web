...
async function gfx1({
	...
	const img = await loadImage('./gfx1.jpg');
	...
	fs.writeFileSync(`./gfx_img1.png`, imgBuffer)
...
async function gfx2({
	...
	const img = await loadImage('./gfx2.jpg');
	const context = canvas.getContext('2d');
	...
	if (text2) {
		...
		context.shadowOffsetX = 6;
		context.shadowOffsetY = 10;
		context.strokeText(text2, (360 * text.length) / text.length, 960, (660 * text.length) / text.length);
		context.stroke();
		context.fillText(text2, (360 * text.length) / text.length, 960, (660 * text.length) / text.length);
	}
	const imgBuffer = canvas.toBuffer('image/png')
	fs.writeFileSync(`./gfx_img2.png`, imgBuffer)
...
async function gfx3({
	...
	const canvas = createCanvas(1600, 1600)
	const img = await loadImage('./gfx3.jpg');
	const context = canvas.getContext('2d');
	context.drawImage(img, 0, 0, 1600, 1600)
	...
	if (text2) {
		...
		context.shadowOffsetX = 6;
		context.shadowOffsetY = 10;
		context.strokeText(text2, (360 * text.length) / text.length, 1200, (670 * text.length) / text.length);
		context.stroke();
		context.fillText(text2, (360 * text.length) / text.length, 1200, (670 * text.length) / text.length);
	}
	const imgBuffer = canvas.toBuffer('image/png')
	fs.writeFileSync(`./gfx_img3.png`, imgBuffer)
