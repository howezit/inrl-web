...
async function igstalk(name) {
	try {
		...
		const {data} = await axios(`https://aemt.me/download/igstalk?username=${name}`);
		const {username,full_name,biography,external_url,is_private,is_verified,profile_pic_url,posts,followers,following } = data.result.user_info;
		const result = {
			status: true,
			username,
			fullname: full_name,
